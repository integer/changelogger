...
confirm($ab="",$bf=false){return" onclick=\"".($bf?"eventStop(event); ":"")."return confirm('".'Opravdu?'.($ab?" (' + $ab + ')":"")."');\"";}function
...
upload_error($k){$ed=($k==UPLOAD_ERR_INI_SIZE?ini_get("upload_max_filesize"):0);return($k?'Nepodařilo se nahrát soubor.'.($ed?" ".sprintf('Maximální povolená velikost souboru je %sB.',$ed):""):'Soubor neexistuje.');}function
...
enum_input($S,$xa,$l,$X,$Cb=null){global$b;preg_match_all("~'((?:[^']|'')*)'~",$l["length"],$Zc);$G=(isset($Cb)?"<label><input type='$S'$xa value='$Cb'".((is_array($X)?in_array($Cb,$X):$X===0)?" checked":"")."><i>".'prázdné'."</i></label>":"");foreach($Zc[1]as$q=>$W){$W=stripcslashes(str_replace("''","'",$W));$Ja=(is_int($X)?$X==$q+1:(is_array($X)?in_array($q+1,$X):$X===$W));$G.=" <label><input type='$S'$xa value='".($q+1)."'".($Ja?' checked':'').'>'.h($b->editVal($W,$l)).'</label>';}return$G;}function
input($l,$X,$o){global$T,$b,$v;$A=h(bracket_escape($l["field"]));echo"<td class='function'>";$Be=($v=="mssql"&&$l["auto_increment"]);if($Be&&!$_POST["save"]){$o=null;}$lc=(isset($_GET["select"])||$Be?array("orig"=>'původní'):array())+$b->editFunctions($l);$xa=" name='fields[$A]'";if($l["type"]=="enum"){echo
...
q($W);}function
check_sqlite_name($A){global$g;$Ub="db|sdb|sqlite";if(!preg_match("~^[^\\0]*\\.($Ub)\$~",$A)){$g->error=sprintf('Prosím použijte jednu z koncovek %s.',str_replace("|",", ",$Ub));return
false;}return
true;}function
...
set_schema($Le){global$g,$T,$df;$G=$g->query("SET search_path TO ".idf_escape($Le));foreach(types()as$S){if(!isset($T[$S])){$T[$S]=0;$df['Uživatelské typy'][]=$S;}}return$G;}function
...
reset($I);}function
support($Wb){return
ereg("view|drop_col|variables|status",$Wb);}$v="oracle";$T=array();$df=array();foreach(array('Čísla'=>array("number"=>38,"binary_float"=>12,"binary_double"=>21),'Datum a čas'=>array("date"=>10,"timestamp"=>29,"interval year"=>12,"interval day"=>28),'Řetězce'=>array("char"=>2000,"varchar2"=>4000,"nchar"=>2000,"nvarchar2"=>4000,"clob"=>4294967295,"nclob"=>4294967295),'Binární'=>array("raw"=>2000,"long raw"=>2147483648,"blob"=>4294967295,"bfile"=>4294967296),)as$w=>$W){$T+=$W;$df[$w]=array_keys($W);}$Rf=array();$Ed=array("=","<",">","<=",">=","!=","LIKE","LIKE %%","IN","IS NULL","NOT LIKE","NOT REGEXP","NOT IN","IS NOT NULL","");$lc=array("length","lower","round","upper");$pc=array("avg","count","count distinct","max","min","sum");$yb=array(array("date"=>"current_date","timestamp"=>"current_timestamp",),array("number|float|double"=>"+/-","date|timestamp"=>"+ interval/- interval","char|clob"=>"||",));}$sb["mssql"]="MS SQL";if(isset($_GET["mssql"])){$ge=array("SQLSRV","MSSQL");define("DRIVER","mssql");if(extension_loaded("sqlsrv")){class
...
selectLinks($kf,$L=""){echo'<p class="tabs">';$Uc=array("select"=>'Vypsat data',"table"=>'Zobrazit strukturu');if(is_view($kf)){$Uc["view"]='Pozměnit pohled';}else{$Uc["create"]='Pozměnit tabulku';}if(isset($L)){$Uc["edit"]='Nová položka';}foreach($Uc
...
selectVal($W,$z,$l){$G=($W!="<i>NULL</i>"&&ereg("char|binary",$l["type"])&&!ereg("var",$l["type"])?"<code>$W</code>":$W);if(ereg('blob|bytea|raw|file',$l["type"])&&!is_utf8($W)){$G=lang(array('%d bajt','%d bajty','%d bajtů'),strlen(html_entity_decode($W,ENT_QUOTES)));}return($z?"<a href='$z'>$G</a>":$G);}function
editVal($W,$l){return(ereg("binary",$l["type"])?reset(unpack("H*",$W)):$W);}function
selectColumnsPrint($J,$f){global$lc,$pc;print_fieldset("select",'Vypsat',$J);$q=0;$kc=array('Funkce'=>$lc,'Agregace'=>$pc);foreach($J
...
as$q=>$t){if($t["type"]=="FULLTEXT"){echo"(<i>".implode("</i>, <i>",array_map('h',$t["columns"]))."</i>) AGAINST"," <input name='fulltext[$q]' value='".h($_GET["fulltext"][$q])."'>",checkbox("boolean[$q]",1,isset($_GET["boolean"][$q]),"BOOL"),"<br>\n";}}$q=0;foreach((array)$_GET["where"]as$W){if("$W[col]$W[val]"!=""&&in_array($W["op"],$this->operators)){echo"<div><select name='where[$q][col]'><option value=''>(".'kdekoliv'.")".optionlist($f,$W["col"],true)."</select>",html_select("where[$q][op]",$this->operators,$W["op"]),"<input name='where[$q][val]' value='".h($W["val"])."'></div>\n";$q++;}}echo"<div><select name='where[$q][col]' onchange='this.nextSibling.nextSibling.onchange();'><option value=''>(".'kdekoliv'.")".optionlist($f,null,true)."</select>",html_select("where[$q][op]",$this->operators,"="),"<input name='where[$q][val]' onchange='selectAddRow(this);'></div>\n","</div></fieldset>\n";}function
selectOrderPrint($Id,$f,$u){print_fieldset("sort",'Seřadit',$Id);$q=0;foreach((array)$_GET["order"]as$w=>$W){if(isset($f[$W])){echo"<div><select name='order[$q]'><option>".optionlist($f,$W,true)."</select>",checkbox("desc[$q]",1,isset($_GET["desc"][$w]),'sestupně')."</div>\n";$q++;}}echo"<div><select name='order[$q]' onchange='selectAddRow(this);'><option>".optionlist($f,null,true)."</select>","<label><input type='checkbox' name='desc[$q]' value='1'>".'sestupně'."</label></div>\n";echo"</div></fieldset>\n";}function
selectLimitPrint($y){echo"<fieldset><legend>".'Limit'."</legend><div>";echo"<input name='limit' size='3' value='".h($y)."'>","</div></fieldset>\n";}function
selectLengthPrint($vf){if(isset($vf)){echo"<fieldset><legend>".'Délka textů'."</legend><div>",'<input name="text_length" size="3" value="'.h($vf).'">',"</div></fieldset>\n";}}function
...
false;}function
messageQuery($E){global$v;static$ab=0;restart_session();$r="sql-".($ab++);$rc=&get_session("queries");if(strlen($E)>1e6){$E=ereg_replace('[\x80-\xFF]+$','',substr($E,0,1e6))."\n...";}$rc[$_GET["db"]][]=$E;return" <a href='#$r' onclick=\"return !toggle('$r');\">".'SQL příkaz'."</a><div id='$r' class='hidden'><pre><code class='jush-$v'>".shorten_utf8($E,1000).'</code></pre><p><a href="'.h(str_replace("db=".urlencode(DB),"db=".urlencode($_GET["db"]),ME).'sql=&history='.(count($rc[$_GET["db"]])-1)).'">'.'Upravit'.'</a></div>';}function
...
explode("/",$G);}function
editInput($O,$l,$xa,$X){if($l["type"]=="enum"){return(isset($_GET["select"])?"<label><input type='radio'$xa value='-1' checked><i>".'původní'."</i></label> ":"").($l["null"]?"<label><input type='radio'$xa value=''".(isset($X)||isset($_GET["select"])?"":" checked")."><i>NULL</i></label> ":"").enum_input("radio",$xa,$l,$X,0);}return"";}function
...
<p class="logout">
';if(DB==""||!$nd){echo"<a href='".h(ME)."sql='".bold(isset($_GET["sql"])).">".'SQL příkaz'."</a>\n";if(support("dump")){echo"<a href='".h(ME)."dump=".urlencode(isset($_GET["table"])?$_GET["table"]:$_GET["select"])."' id='dump'".bold(isset($_GET["dump"])).">".'Export'."</a>\n";}}echo'<input type="submit" name="logout" value="Odhlásit" onclick="eventStop(event);">
<input type="hidden" name="token" value="',$R,'">
</p>
</form>
<form action="">
<p>
';hidden_fields_get();echo($i?html_select("db",array(""=>"(".'databáze'.")")+$i,DB,"this.form.submit();"):'<input name="db" value="'.h(DB).'">'),'<input type="submit" value="Vybrat"',($i?" class='hidden'":""),' onclick="eventStop(event);">
';if($nd!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".html_select("ns",array(""=>"(".'schéma'.")")+schemas(),$_GET["ns"],"this.form.submit();");if($_GET["ns"]!=""){set_schema($_GET["ns"]);}}if($_GET["ns"]!==""&&!$nd){echo'<p><a href="'.h(ME).'create="'.bold($_GET["create"]==="").">".'Vytvořit novou tabulku'."</a>\n";$Q=tables_list();if(!$Q){echo"<p class='message'>".'Žádné tabulky.'."\n";}else{$this->tablesPrint($Q);$Uc=array();foreach($Q
...
as$w=>$W){if(!isset($W)){$W="<i>NULL</i>";}elseif($Da[$w]&&!is_utf8($W)){$W="<i>".lang(array('%d bajt','%d bajty','%d bajtů'),strlen($W))."</i>";}elseif(!strlen($W)){$W="&nbsp;";}else{$W=h($W);if($T[$w]==254){$W="<code>$W</code>";}}if(isset($Uc[$w])&&!$f[$Uc[$w]]){if($uc){$z=$Uc[$w].urlencode($H[array_search("table=",$Uc)]);}else{$z="edit=".urlencode($Uc[$w]);foreach($u[$Uc[$w]]as$Ma=>$Hc){$z.="&where".urlencode("[".bracket_escape($Ma)."]")."=".urlencode($H[$Hc]);}}$W="<a href='".h(ME.$z)."'>$W</a>";}echo"<td>$W";}}echo($q?"</table>":"<p class='message'>".'Žádné řádky.')."\n";}function
...
edit_type($w,$l,$Oa,$ec=array()){global$df,$T,$Rf,$Ad;echo'<td><select name="',$w,'[type]" class="type" onfocus="lastType = selectValue(this);" onchange="editingTypeChange(this);">',optionlist((!$l["type"]||isset($T[$l["type"]])?array():array($l["type"]))+$df+($ec?array('Cizí klíče'=>$ec):array()),$l["type"]),'</select>
<td><input name="',$w,'[length]" value="',h($l["length"]),'" size="3" onfocus="editingLengthFocus(this);"><td class="options">',"<select name='$w"."[collation]'".(ereg('(char|text|enum|set)$',$l["type"])?"":" class='hidden'").'><option value="">('.'porovnávání'.')'.optionlist($Oa,$l["collation"]).'</select>',($Rf?"<select name='$w"."[unsigned]'".(!$l["type"]||ereg('(int|float|double|decimal)$',$l["type"])?"":" class='hidden'").'><option>'.optionlist($Rf,$l["unsigned"]).'</select>':''),($ec?"<select name='$w"."[on_delete]'".(ereg("`",$l["type"])?"":" class='hidden'")."><option value=''>(".'Při smazání'.")".optionlist(explode("|",$Ad),$l["on_delete"])."</select> ":" ");}function
...
',(support("comment")?"<td".($Ta?"":" class='hidden'").">".'Komentář':"");}echo'<td>',"<input type='image' name='add[".(support("move_col")?0:count($m))."]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.4' alt='+' title='".'Přidat další'."'>",'<script type="text/javascript">row_count = ',count($m),';</script>
</thead>
...
',(support("comment")?"<td".($Ta?"":" class='hidden'")."><input name='fields[$q][comment]' value='".h($l["comment"])."' maxlength='255'>":"");}echo"<td>",(support("move_col")?"<input type='image' name='add[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=plus.gif&amp;version=3.3.4' alt='+' title='".'Přidat další'."' onclick='return !editingAddRow(this, $va, 1);'>&nbsp;"."<input type='image' name='up[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=up.gif&amp;version=3.3.4' alt='^' title='".'Přesunout nahoru'."'>&nbsp;"."<input type='image' name='down[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=down.gif&amp;version=3.3.4' alt='v' title='".'Přesunout dolů'."'>&nbsp;":""),($Ld==""||support("drop_col")?"<input type='image' name='drop_col[$q]' src='".h(preg_replace("~\\?.*~","",ME))."?file=cross.gif&amp;version=3.3.4' alt='x' title='".'Odebrat'."' onclick='return !editingRemoveRow(this);'>":""),"\n";}}function
...
<input type="hidden" name="token" value="',$R,'">
',checkbox("error_stops",1,$_POST["error_stops"],'Zastavit při chybě')."\n",checkbox("only_errors",1,$_POST["only_errors"],'Zobrazit pouze chyby')."\n";print_fieldset("webfile",'Ze serveru',$_POST["webfile"],"document.getElementById('form')['only_errors'].checked = true; ");$Ua=array();foreach(array("gz"=>"zlib","bz2"=>"bz2")as$w=>$W){if(extension_loaded($W)){$Ua[]=".$w";}}echo
sprintf('Soubor %s na webovém serveru',"<code>adminer.sql".($Ua?"[".implode("|",$Ua)."]":"")."</code>"),' <input type="submit" name="webfile" value="'.'Spustit soubor'.'">',"</div></fieldset>\n";if($rc){print_fieldset("history",'Historie',$_GET["history"]!="");foreach($rc
as$w=>$W){echo'<a href="'.h(ME."sql=&history=$w").'">'.'Upravit'."</a> <code class='jush-$v'>".shorten_utf8(ltrim(str_replace("\n"," ",str_replace("\r","",preg_replace('~^(#|-- ).*~m','',$W)))),80,"</code>")."<br>\n";}echo"<input type='submit' name='clear' value='".'Vyčistit'."'>\n","<a href='".h(ME."sql=&history=all")."'>".'Upravit vše'."</a>\n","</div></fieldset>\n";}echo'
</form>
...
as$A=>$l){if(!isset($l["privileges"][$Sf?"update":"insert"])||$b->fieldName($l)==""){unset($m[$A]);}}if($_POST&&!$k&&!isset($_GET["select"])){$Vc=$_POST["referer"];if($_POST["insert"]){$Vc=($Sf?null:$_SERVER["REQUEST_URI"]);}elseif(!ereg('^.+&select=.+$',$Vc)){$Vc=ME."select=".urlencode($a);}if(isset($_POST["delete"])){query_redirect("DELETE".limit1("FROM ".table($a)," WHERE $Z"),$Vc,'Položka byla smazána.');}else{$L=array();foreach($m
as$A=>$l){$W=process_input($l);if($W!==false&&$W!==null){$L[idf_escape($A)]=($Sf?"\n".idf_escape($A)." = $W":$W);}}if($Sf){if(!$L){redirect($Vc);}query_redirect("UPDATE".limit1(table($a)." SET".implode(",",$L),"\nWHERE $Z"),$Vc,'Položka byla aktualizována.');}else{$F=insert_into($a,$L);$Oc=($F?last_id():0);queries_redirect($Vc,sprintf('Položka%s byla vložena.',($Oc?" $Oc":"")),$F);}}}$lf=$b->tableName(table_status($a));page_header(($Sf?'Upravit':'Vložit'),$k,array("select"=>array($a,$lf)),$lf);$H=null;if($_POST["save"]){$H=(array)$_POST["fields"];}elseif($Z){$J=array();foreach($m
as$A=>$l){if(isset($l["privileges"]["select"])){$J[]=($_POST["clone"]&&$l["auto_increment"]?"'' AS ":(ereg("enum|set",$l["type"])?"1*".idf_escape($A)." AS ":"")).idf_escape($A);}}$H=array();if($J){$I=get_rows("SELECT".limit(implode(", ",$J)." FROM ".table($a)," WHERE $Z",(isset($_GET["select"])?2:1)));$H=(isset($_GET["select"])&&count($I)!=1?null:reset($I));}}if($H===false){echo"<p class='error'>".'Žádné řádky.'."\n";}echo'
<form action="" method="post" enctype="multipart/form-data" id="form">
...
';if($m){echo"<input type='submit' value='".'Uložit'."'>\n";if(!isset($_GET["select"])){echo"<input type='submit' name='insert' value='".($Sf?'Uložit a pokračovat v editaci':'Uložit a vložit další')."' title='Ctrl+Shift+Enter'>\n";}}echo($Sf?"<input type='submit' name='delete' value='".'Smazat'."' onclick=\"return confirm('".'Opravdu?'."');\">\n":($_POST||!$m?"":"<script type='text/javascript'>document.getElementById('form').getElementsByTagName('td')[1].firstChild.focus();</script>\n"));if(isset($_GET["select"])){hidden_fields(array("check"=>(array)$_POST["check"],"clone"=>$_POST["clone"],"all"=>$_POST["all"]));}echo'<input type="hidden" name="referer" value="',h(isset($_POST["referer"])?$_POST["referer"]:$_SERVER["HTTP_REFERER"]),'">
...
';if($a==""&&!$_POST){?><script type='text/javascript'>document.getElementById('form')['name'].focus();</script><?php }echo($Fb?html_select("Engine",array(""=>"(".'úložiště'.")")+$Fb,$H["Engine"]):""),' ',($Oa&&!ereg("sqlite|mssql",$v)?html_select("Collation",array(""=>"(".'porovnávání'.")")+$Oa,$H["Collation"]):""),' <input type="submit" value="Uložit">
...
',(support("comment")?checkbox("comments",1,$Ta,'Komentář',"columnShow(this.checked, 6); toggle('Comment'); if (this.checked) this.form['Comment'].focus();",true).' <input id="Comment" name="Comment" value="'.h($H["Comment"]).'" maxlength="60"'.($Ta?'':' class="hidden"').'>':''),'<p>
<input type="submit" value="Uložit">
';if($_GET["create"]!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
';if(support("partitioning")){$Wd=ereg('RANGE|LIST',$H["partition_by"]);print_fieldset("partition",'Rozdělit podle',$H["partition_by"]);echo'<p>
',html_select("partition_by",array(-1=>"")+$Vd,$H["partition_by"],"partitionByChange(this);"),'(<input name="partition" value="',h($H["partition"]),'">)
Oddíly: <input name="partitions" size="2" value="',h($H["partitions"]),'"',($Wd||!$H["partition_by"]?" class='hidden'":""),'>
...
';}elseif(isset($_GET["database"])){if($_POST&&!$k&&!isset($_POST["add_x"])){restart_session();$A=trim($_POST["name"]);if($_POST["drop"]){$_GET["db"]="";queries_redirect(remove_from_uri("db|database"),'Databáze byla odstraněna.',drop_databases(array(DB)));}elseif(DB!==$A){if(DB!=""){$_GET["db"]=$A;queries_redirect(preg_replace('~db=[^&]*&~','',ME)."db=".urlencode($A),'Databáze byla přejmenována.',rename_database($A,$_POST["collation"]));}else{$i=explode("\n",str_replace("\r","",$A));$ef=true;$Nc="";foreach($i
as$j){if(count($i)==1||$j!=""){if(!create_database($j,$_POST["collation"])){$ef=false;}$Nc=$j;}}queries_redirect(ME."db=".urlencode($Nc),'Databáze byla vytvořena.',$ef);}}else{if(!$_POST["collation"]){redirect(substr(ME,0,-1));}query_redirect("ALTER DATABASE ".idf_escape($A).(eregi('^[a-z0-9_]+$',$_POST["collation"])?" COLLATE $_POST[collation]":""),substr(ME,0,-1),'Databáze byla změněna.');}}page_header(DB!=""?'Pozměnit databázi':'Vytvořit databázi',$k,array(),DB);$Oa=collations();$A=DB;$Na=null;if($_POST){$A=$_POST["name"];$Na=$_POST["collation"];}elseif(DB!=""){$Na=db_collation(DB,$Oa);}elseif($v=="sql"){foreach(get_vals("SHOW GRANTS")as$p){if(preg_match('~ ON (`(([^\\\\`]|``|\\\\.)*)%`\\.\\*)?~',$p,$_)&&$_[1]){$A=stripcslashes(idf_unescape("`$_[2]`"));break;}}}echo'
<form action="" method="post">
<p>
',($_POST["add_x"]||strpos($A,"\n")?'<textarea id="name" name="name" rows="10" cols="40">'.h($A).'</textarea><br>':'<input id="name" name="name" value="'.h($A).'" maxlength="64">')."\n".($Oa?html_select("collation",array(""=>"(".'porovnávání'.")")+$Oa,$Na):"");?>
...
';}elseif(isset($_GET["call"])){$da=$_GET["call"];page_header('Zavolat'.": ".h($da),$k);$Fe=routine($da,(isset($_GET["callf"])?"FUNCTION":"PROCEDURE"));$xc=array();$Pd=array();foreach($Fe["fields"]as$q=>$l){if(substr($l["inout"],-3)=="OUT"){$Pd[$q]="@".idf_escape($l["field"])." AS ".idf_escape($l["field"]);}if(!$l["inout"]||substr($l["inout"],0,2)=="IN"){$xc[]=$q;}}if(!$k&&$_POST){$Ha=array();foreach($Fe["fields"]as$w=>$l){if(in_array($w,$xc)){$W=process_input($l);if($W===false){$W="''";}if(isset($Pd[$w])){$g->query("SET @".idf_escape($l["field"])." = $W");}}$Ha[]=(isset($Pd[$w])?"@".idf_escape($l["field"]):$W);}$E=(isset($_GET["callf"])?"SELECT":"CALL")." ".idf_escape($da)."(".implode(", ",$Ha).")";echo"<p><code class='jush-$v'>".h($E)."</code> <a href='".h(ME)."sql=".urlencode($E)."'>".'Upravit'."</a>\n";if(!$g->multi_query($E)){echo"<p class='error'>".error()."\n";}else{$h=connect();if(is_object($h)){$h->select_db(DB);}do{$F=$g->store_result();if(is_object($F)){select($F,$h);}else{echo"<p class='message'>".lang(array('Procedura byla zavolána, byl změněn %d záznam.','Procedura byla zavolána, byly změněny %d záznamy.','Procedura byla zavolána, bylo změněno %d záznamů.'),$g->affected_rows)."\n";}}while($g->next_result());if($Pd){select($g->query("SELECT ".implode(", ",$Pd)));}}}echo'
<form action="" method="post">
...
';}elseif(isset($_GET["foreign"])){$a=$_GET["foreign"];if($_POST&&!$k&&!$_POST["add"]&&!$_POST["change"]&&!$_POST["change-js"]){if($_POST["drop"]){query_redirect("ALTER TABLE ".table($a)."\nDROP ".($v=="sql"?"FOREIGN KEY ":"CONSTRAINT ").idf_escape($_GET["name"]),ME."table=".urlencode($a),'Cizí klíč byl odstraněn.');}else{$Ve=array_filter($_POST["source"],'strlen');ksort($Ve);$sf=array();foreach($Ve
...
<p>
Při smazání: ',html_select("on_delete",array(-1=>"")+explode("|",$Ad),$H["on_delete"]),' Při změně: ',html_select("on_update",array(-1=>"")+explode("|",$Ad),$H["on_update"]),'<p>
<input type="submit" value="Uložit">
...
<tr><th>Každých<td><input name="INTERVAL_VALUE" value="',h($H["INTERVAL_VALUE"]),'" size="6"> ',html_select("INTERVAL_FIELD",$Ec,$H["INTERVAL_FIELD"]),'<tr><th>Stav<td>',html_select("STATUS",$af,$H["STATUS"]),'<tr><th>Komentář<td><input name="EVENT_COMMENT" value="',h($H["EVENT_COMMENT"]),'" maxlength="64">
...
<input type="submit" value="Uložit">
';if($da!=""){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}if($ub){echo'<input type="hidden" name="dropped" value="1">';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["sequence"])){$fa=$_GET["sequence"];if($_POST&&!$k){$z=substr(ME,0,-1);$A=trim($_POST["name"]);if($_POST["drop"]){query_redirect("DROP SEQUENCE ".idf_escape($fa),$z,'Sekvence byla odstraněna.');}elseif($fa==""){query_redirect("CREATE SEQUENCE ".idf_escape($A),$z,'Sekvence byla vytvořena.');}elseif($fa!=$A){query_redirect("ALTER SEQUENCE ".idf_escape($fa)." RENAME TO ".idf_escape($A),$z,'Sekvence byla změněna.');}else{redirect($z);}}page_header($fa!=""?'Pozměnit sekvenci'.": ".h($fa):'Vytvořit sekvenci',$k);$H=$_POST;if(!$H){$H=array("name"=>$fa);}echo'
<form action="" method="post">
<p><input name="name" value="',h($H["name"]),'">
<input type="submit" value="Uložit">
';if($fa!=""){echo"<input type='submit' name='drop' value='".'Odstranit'."'".confirm().">\n";}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["type"])){$ga=$_GET["type"];if($_POST&&!$k){$z=substr(ME,0,-1);if($_POST["drop"]){query_redirect("DROP TYPE ".idf_escape($ga),$z,'Typ byl odstraněn.');}else{query_redirect("CREATE TYPE ".idf_escape(trim($_POST["name"]))." $_POST[as]",$z,'Typ byl vytvořen.');}}page_header($ga!=""?'Pozměnit typ'.": ".h($ga):'Vytvořit typ',$k);$H=$_POST;if(!$H){$H=array("as"=>"AS ");}echo'
<form action="" method="post">
<p>
...
as$W){if($W[1]!="USAGE"){$mc["$_[2]$W[2]"][$W[1]]=true;}if(ereg(' WITH GRANT OPTION',$H[0])){$mc["$_[2]$W[2]"]["GRANT OPTION"]=true;}}}if(preg_match("~ IDENTIFIED BY PASSWORD '([^']+)~",$H[0],$_)){$yd=$_[1];}}}if($_POST&&!$k){$zd=(isset($_GET["host"])?q($ha)."@".q($_GET["host"]):"''");$td=q($_POST["user"])."@".q($_POST["host"]);$Zd=q($_POST["pass"]);if($_POST["drop"]){query_redirect("DROP USER $zd",ME."privileges=",'Uživatel byl odstraněn.');}else{$db=false;if($zd!=$td){$db=queries(($g->server_info<5?"GRANT USAGE ON *.* TO":"CREATE USER")." $td IDENTIFIED BY".($_POST["hashed"]?" PASSWORD":"")." $Zd");$k=!$db;}elseif($_POST["pass"]!=$yd||!$_POST["hashed"]){queries("SET PASSWORD FOR $td = ".($_POST["hashed"]?$Zd:"PASSWORD($Zd)"));}if(!$k){$Ce=array();foreach($sd
...
as$wd=>$Ce){if(preg_match('~^(.+)(\\(.*\\))?$~U',$wd,$_)){grant("REVOKE",array_keys($Ce),$_[2]," ON $_[1] FROM $td");}}}}queries_redirect(ME."privileges=",(isset($_GET["host"])?'Uživatel byl změněn.':'Uživatel byl vytvořen.'),!$k);if($db){$g->query("DROP USER $td");}}}page_header((isset($_GET["host"])?'Uživatel'.": ".h("$ha@$_GET[host]"):'Vytvořit uživatele'),$k,array("privileges"=>array('','Oprávnění')));if($_POST){$H=$_POST;$mc=$sd;}else{$H=$_GET+array("host"=>$g->result("SELECT SUBSTRING_INDEX(CURRENT_USER, '@', -1)"));$H["pass"]=$yd;if($yd!=""){$H["hashed"]=true;}$mc[(DB!=""&&!isset($_GET["host"])?idf_escape(addcslashes(DB,"%_")):"").".*"]=array();}echo'<form action="" method="post">
<table cellspacing="0">
...
';echo"<table cellspacing='0'>\n","<thead><tr><th colspan='2'><a href='http://dev.mysql.com/doc/refman/".substr($g->server_info,0,3)."/en/grant.html#priv_level' target='_blank' rel='noreferrer'>".'Oprávnění'."</a>";$q=0;foreach($mc
as$wd=>$p){echo'<th>'.($wd!="*.*"?"<input name='objects[$q]' value='".h($wd)."' size='10'>":"<input type='hidden' name='objects[$q]' value='*.*' size='10'>*.*");$q++;}echo"</thead>\n";foreach(array(""=>"","Server Admin"=>'Server',"Databases"=>'Databáze',"Tables"=>'Tabulka',"Columns"=>'Sloupec',"Procedures"=>'Procedura',)as$Ya=>$nb){foreach((array)$ne[$Ya]as$me=>$Sa){echo"<tr".odd()."><td".($nb?">$nb<td":" colspan='2'").' lang="en" title="'.h($Sa).'">'.h($me);$q=0;foreach($mc
as$wd=>$p){$A="'grants[$q][".h(strtoupper($me))."]'";$X=$p[strtoupper($me)];if($Ya=="Server Admin"&&$wd!=(isset($mc["*.*"])?"*.*":".*")){echo"<td>&nbsp;";}elseif(isset($_GET["grant"])){echo"<td><select name=$A><option><option value='1'".($X?" selected":"").">".'Povolit'."<option value='0'".($X=="0"?" selected":"").">".'Zakázat'."</select>";}else{echo"<td align='center'><input type='checkbox' name=$A value='1'".($X?" checked":"").($me=="All privileges"?" id='grants-$q-all'":($me=="Grant option"?"":" onclick=\"if (this.checked) formUncheck('grants-$q-all');\"")).">";}$q++;}}}echo"</table>\n",'<p>
<input type="submit" value="Uložit">
';if(isset($_GET["host"])){echo'<input type="submit" name="drop" value="Odstranit"',confirm(),'>';}echo'<input type="hidden" name="token" value="',$R,'">
</form>
';}elseif(isset($_GET["processlist"])){if(support("kill")&&$_POST&&!$k){$Kc=0;foreach((array)$_POST["kill"]as$W){if(queries("KILL ".(+$W))){$Kc++;}}queries_redirect(ME."processlist=",lang(array('Byl ukončen %d proces.','Byly ukončeny %d procesy.','Bylo ukončeno %d procesů.'),$Kc),$Kc||!$_POST["kill"]);}page_header('Seznam procesů',$k);echo'
<form action="" method="post">
...
as$w=>$W){echo"<td>".(($v=="sql"?$w=="Info"&&$W!="":$w=="current_query"&&$W!="<IDLE>")?"<code class='jush-$v'>".shorten_utf8($W,100,"</code>").' <a href="'.h(ME.($H["db"]!=""?"db=".urlencode($H["db"])."&":"")."sql=".urlencode($W)).'">'.'Upravit'.'</a>':nbsp($W));}echo"\n";}echo'</table>
<script type=\'text/javascript\'>tableCheck();</script>
<p>
